...
// Wrappers
JEvent* MsgParser::getEventById(unsigned int id, unsigned int subID){return v->getEventById(id, subID);}
JEvent* MsgParser::addEvent(JEvent* e, int layerIndex, int index, bool bOldVersion){return v->addEvent(e, layerIndex, e->id, bOldVersion);}
...
void MsgParser::getFreePointers(string host, int port){v->getFreePointers(host, port);}
...
void MsgParser::setCirclularMaskState(bool bState){v->bDrawCirclularMask = bState;}
