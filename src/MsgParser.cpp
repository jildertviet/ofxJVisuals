...
MsgParser::MsgParser(ofxJVisuals *v, bool bUseSC) : v(v), bUseSC(bUseSC) {
  ...
  vector<string> typeKeys = {
      "JRectangle",   "JVecField",   "JMirror",    "JEllipse",
      "JVorm",        "JSpaceCube",  "JMultiMesh", "JParticles",
      "JText",        "JMesh",       "JImage",     "JDivisionGrid",
      "JVideoPlayer", "JShaderTest", "JNoise",     "JLinesFalling",
      "JIFLogo",      "JPhysarum",   "JModifier",  "JCircle",
      "JLine",        "JShaderLines"};
  for (short i = 0; i < typeKeys.size(); i++)
    types[typeKeys[i]] = i + 1;

  vector<string> valueKeys = {
      "color",     "loc",        "size",        "bMove",          "speed",
      "direction", "lineLength", "complexity",  "density",        "angle",
      "customArg", "bForm",      "maxSpeed",    "lijnMax",        "lineWidth",
      "mode",      "text",       "path",        "rotation",       "drawMode",
      "alpha",     "quadColor",  "sensorAngle", "sensorDistance", "turnAngle",
      "decay",     "deposit",    "balance",     "fill",           "radius",
      "evolve",    "zoom",       "bus"};
  for (short i = 0; i < valueKeys.size(); i++)
    values[valueKeys[i]] = i + 1;

  vector<string> envValueKeys = {
      "width", "height", "brightness", "lijnMax",    "x",           "y",
      "speed", "z",      "depth",      "randomPoly", "selectedPoly"};
...
void MsgParser::connectToSuperCollider() {
  ...
  ofAddListener(ofxOscEvent::packetIn, this,
                &MsgParser::onSuperColliderMessageReceived);
}
...
bool MsgParser::parseMsg(ofxOscMessage &m) {
  ...
  case 11: { // linkVecField
    ...
    if (e) {
      if (e->type == "particleSystem")
        ((particleSystem *)e)
            ->setVecField((JVecField *)getEventById(m.getArgAsInt(1)));
    }
#endif
  } break;
  case 12: // getFreePointers
    getFreePointers(m.getRemoteHost(),
                    m.getArgAsInt(1)); // Sends to 6063, w/ SCsender
    break;
  ...
  case 21: // camRotateAround
    getCam()->rotateAroundDeg(
        m.getArgAsFloat(0),
        glm::vec3(m.getArgAsFloat(1), m.getArgAsFloat(2), m.getArgAsFloat(3)),
        glm::vec3(0, 0, 0));
    break;
...
bool MsgParser::make(ofxOscMessage &m) {
  cout << m << endl;
  cout << "Make " << m.getArgAsString(0) << " with ID: " << m.getArgAsInt(1)
       << endl;
  JEvent *e = nullptr;
  int layer = 2; // Default
  if (m.getNumArgs() > 2) {
...
//            e = (JEvent*)new JParticles();
#if USE_OPENCL
    e = (JEvent *)new particleSystem(
        m.getArgAsInt(4), ofVec2f(m.getArgAsInt(5), m.getArgAsInt(6)),
        ofColor::white, m.getArgAsInt(7));
#endif
  ...
  case 9: {
    ...
    if (layer == VisualizerLayer::NON_CAM_BACK ||
        layer == VisualizerLayer::NON_CAM_FRONT) {
  ...
  case 18: {
    ...
    //                    if(i < 512){
    //                        pixels.setColor(i, j, ofFloatColor(0.5, 0.5,
    //                        0.5));
    //                    } else{
    //                        pixels.setColor(i, j, ofFloatColor(0.0, 0.0,
    //                        0.0));
    //                    }
    //                }
    //            }
  ...
  // Send back pointer (as long) to SC. Issue: this takes time. When you call
  // event.create(); event.setLoc([0,0]); from SC that second call won't have
  // this pointer yet...
  //    ofxOscMessage n;
  //    n.setAddress("/makeConfirm");
  //    n.addIntArg(m.getArgAsInt(1)); // ID from SC
  //    long pointerToInt = reinterpret_cast<long>(e); // This int can later be
  //    casted to an JEvent*, but check if it's allocated/in use before using!
...
void MsgParser::setVal(
    ofxOscMessage &m) { // Default: /setVal, 0, "size", 100, 200
  JEvent *e = getEventById(m.getArgAsInt(0));
  // Default: /setVal, a, b, "size", 100, 200: a and b are two uint_32 forming a
  // long together, that can be casted to a JEvent*
  //    long pointerAsInt = (long)m.getArgAsInt(1) << 32 | m.getArgAsInt(2) &
  //    0xFFFFFFFFL; JEvent* f = reinterpret_cast<JEvent*>(pointerAsInt);
  if (e) {
    cout << "Event found, id: " << m.getArgAsInt(0) << ", addr: " << e << endl;
    switch (values[m.getArgAsString(1)]) {
    case 1: // color
            // if(m.getNumArgs() == 6){ // Set first, global, color
      e->setColor(ofColor(m.getArgAsInt(2), m.getArgAsInt(3), m.getArgAsInt(4),
                          m.getArgAsInt(5)));
      // } else if (m.getNumArgs() == 7){ // Set color @ index of event
      // e->setColor(ofColor(m.getArgAsInt(2), m.getArgAsInt(3),
      // m.getArgAsInt(4), m.getArgAsInt(5)), m.getArgAsInt(6));
      // }
      break;
    case 2:                      // loc
      if (m.getNumArgs() == 4) { // 2D
        ...
        e->setLoc(
            ofVec3f(m.getArgAsInt(2), m.getArgAsInt(3), m.getArgAsInt(4)));
      }
      break;
    case 3:                      // size
      if (m.getNumArgs() == 4) { // 2D
    ...
    case 6: { // Direction
      if (m.getNumArgs() >= 5) {
        e->direction = glm::vec3(m.getArgAsFloat(2), m.getArgAsFloat(3),
                                 m.getArgAsFloat(4));
      } else {
    ...
    case 9: { // density
      if (m.getArgAsString(2) == "JVecField")
        ((JVecField *)e)
            ->setDensity(glm::vec2(m.getArgAsInt(3), m.getArgAsInt(4)));
    } break;
    case 10: { // angle
    ...
    case 19: // rotation
      e->rotation =
          ofVec3f(m.getArgAsFloat(2), m.getArgAsFloat(3), m.getArgAsFloat(4));
      break;
    case 20: { // drawMode
      if (e->type == "JVecField")
    ...
    case 22:
      if (e->type == "JRectangle") {
        ...
        ofColor a =
            ofColor(m.getArgAsInt(2), m.getArgAsInt(3), m.getArgAsInt(4));
        cout << "a: " << a << endl;
        ((JRectangle *)e)
            ->setQuadColor(
                a,
                ...
                ofColor(m.getArgAsInt(11), m.getArgAsInt(12),
                        m.getArgAsInt(13)));
...
void MsgParser::addTo(ofxOscMessage &m) {
  ...
  if (e) {
    ...
    case 1: { // color
      ofColor c = e->color;
      c += ofColor(m.getArgAsInt(2), m.getArgAsInt(3), m.getArgAsInt(4),
                   m.getArgAsInt(5));
      e->setColor(c);
    } break;
    case 2: {                    // loc
      if (m.getNumArgs() == 4) { // 2d
        ...
        e->setLoc(e->loc + ofVec3f(m.getArgAsInt(2), m.getArgAsInt(3),
                                   m.getArgAsInt(4)));
      }
    } break;
    case 3:                      // size
...
void MsgParser::addEnv(ofxOscMessage &m) {
  ...
  if (!e)
    return;
  vector<float> times = {m.getArgAsFloat(2), m.getArgAsFloat(3),
                         m.getArgAsFloat(4)};
  vector<float> values = {m.getArgAsFloat(5), m.getArgAsFloat(6),
                          m.getArgAsFloat(7), m.getArgAsFloat(8)};
  ...
  case 3: { // Brightness
    for (int i = 0; i < values.size();
         i++) { // Replace -1 with current alpha value
      if (values[i] == -1)
        values[i] = e->color.a;
    }
...
void MsgParser::onSuperColliderMessageReceived(
    ofxOscMessage &
        m) { // 2: event id, 3: param id, 4: value, 5: (optional) type (r,g,b,a)
  ...
  if (a == "/mapVal") {
    ...
    for (int i = 0; i < numValues; i++) {
      ...
      if (type == ofxOscArgType::OFXOSC_TYPE_INT32) {
        e->mapValues[m.getArgAsInt(4 + i)]->setVal(
            m.getArgAsInt32(4 + numValues + i));
      } else if (type == ofxOscArgType::OFXOSC_TYPE_FLOAT) {
        e->mapValues[m.getArgAsInt(4 + i)]->setVal(
            m.getArgAsFloat(4 + numValues + i));
      }
    }
  } else if (a == "/update") {
    updateValues(m);
...
bool MsgParser::create(ofxOscMessage &m) {
  cout << m << endl;
  cout << "ID: " << m.getArgAsInt(0)
       << ", type: " << (int)encodedIntToChar(m.getArgAsInt(1), 0)
       << ", subID: " << (int)encodedIntToChar(m.getArgAsInt(1), 1) << endl;
  JEvent *e = nullptr;
  ...
  case jevent::JLine:
    e = new JLine();
    break;
  case jevent::JWaveform:
    e = new JWaveform();
    break;
  case jevent::JModifierRedraw:
...
bool MsgParser::trigger(ofxOscMessage &m) {
  ...
  // cout << "Target: " << targetID << ", " << m.getArgAsInt(3) << endl; // ID,
  // subID
  ...
  if (target) {
    ...
    cout << "Target not found, " << targetID << ", " << m.getArgAsInt(3)
         << endl;
    return false;
  }
  return false;
...
bool MsgParser::setBuffer(ofxOscMessage &m) {
  ...
  if (target) {
    ...
    cout << "Target not found, " << targetID << ", " << m.getArgAsInt(1)
         << endl;
    return false;
  }
  return false;
