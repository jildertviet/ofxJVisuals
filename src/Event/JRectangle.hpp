...
class JRectangle : public JPrimitive,
                   public JThickLine { // Also inherits from Event
public:
  JRectangle();
  JRectangle(float millisTime, glm::vec3 loc, glm::vec3 size,
             ofColor color = ofColor(255, 255), float attack = 100,
             float release = 100, glm::vec3 direction = glm::vec3(-1, 0, 0),
             bool move = true); // Temporary

  JRectangle(glm::vec3 loc,
             glm::vec3 size = glm::vec3(ofGetWindowSize(), 0.0)); // Infite
  ...
  vector<JRectangle *> *v = nullptr; // Zou ook nog in een vector van pointers
                                     // naar vector<rect*> kunnen...
