...
JVorm::JVorm(uint8 numSides, int sideDiv, float radius, glm::vec3 loc,
             bool on_destination) {
  setType("JVorm");
  this->radius = radius;
  this->loc = loc;
...
void JVorm::customFive(float *v) { instantFormJVorm(); }

void JVorm::makeCoordinates(uint8 numSides, int sideDiv, float radius_,
                            bool onDestination) {
  coordinates.clear();
  particles.clear();
  ...
  if (numSides < 2 || sideDiv < 2) {
    cout << "Number of sides or side division is too low, number of sides "
            "should be >2, side division >2"
         << endl;
    if (numSides < 2) {
      numSides = 2;
    }
  ...
  switch (numSides) { // angleOffset is changed, so a rectangle will have
  ...
  for (uint8 i = 0; i < numSides; i++) { // calculate coordinates
    coordinates.push_back(glm::vec3(
        loc.x +
            (cos((angleOffset + (angle * i)) * (PI / 180.)) * radius), // xPos
        loc.y +
            (sin((angleOffset + (angle * i)) * (PI / 180.)) * radius), // yPos
        0));
  }
  ...
  for (short i = 0; i < coordinates.size(); i++) {
    particles.push_back(
        new JParticle(&(coordinates[i]), !onDestination)); // Particles
...
void JVorm::changeRadius(
    float radius_) { // Could be improved by only adding radius increasement to
                     // coordinates ...
...
void JVorm::rotateCoordinates(int places, int direction) {
  ...
  if (direction > 0) {
    rotate(coordinates.begin(),
           coordinates.begin() + (places % coordinates.size()),
           coordinates.end());
  } else {
    if (direction < 0) {
...
void JVorm::tussenCoordinaten() {
  for (uint8 i = 0; i < numSides; i++) {
    for (uint8 j = 0; j < sideDiv - 1; j++) {
      //            int index = numSides+(   (i*((int)side_division-1))+j ); //
      //            KLOPT

      glm::vec3 delta =
          coordinates[((i + 1) % numSides)] - coordinates[(i % numSides)];
      delta = delta * ((1 / sideDiv) * (j + 1));
      coordinates.push_back(coordinates[i] + delta);
      // *(particlespointer+index) = new Particle(coordinates, index);
      // cout << "Coordinaten_" << index << " " << coordinates[index] << ", met
      // i: " << i << " en j: " << j  << endl;
    }
  }
  ...
  for (short i = 1; i < particles.size();
       i++) { // Start with i=1, because 0th stays 0th
    if (i % (int)sideDiv == 0) {
      ...
      int indexTempCoordinates =
          numSides + (side * (sideDiv - 1)) + nth_division;
      //            cout << "indexTempCoordinates: " << indexTempCoordinates-1
      //            << endl;
      coordinates[i] = temp[indexTempCoordinates - 1];
      //            cout << "Setting " << i << " to: " << coordinates[i] <<
      //            endl;
    }
  }
  vector<glm::vec3>().swap(temp);
...
bool JVorm::checkIfInFormation() {
  ...
  if (ofGetFrameNum() % 2 == 1) {
    for (int i = 0; i < coordinates.size(); i++) {
      float distance =
          glm::distance(particles[i]->loc, (*(*(particles[i])).destination));
...
void JVorm::formJVorm() { changeStateOfParticles(1); }
...
void JVorm::unConnect() { // Say to all connected objects: remove me from your
                          // vector
...
void JVorm::connectWith() {
  // If A and B are not connected: draw lines from A to B, set both to
  // 'connected' on both objects. B will do nothing with A.
  for (int i = 0; i < connections.size(); i++) {
    if (connections[i].connected == false) {
      ...
      // CONNECT
      // Returns the pointer from the other object, pointing to this object   (A
      // finds A* in B)
      JVorm::connection *b = connections[i].vorm->findSelf(this);

      //            cout << "B points to A with adress: " << b->JVorm << " is
      //            this " << this << endl;
...
void JVorm::addConnection(JVorm *JVorm, bool repeat) { // repeat = true
  ...
  if (repeat) {
    JVorm->addConnection(
        this, false); // Second time, only the first 4 lines will be evaluated
  }
...
void JVorm::addVector(vector<JVorm *> *v) { this->connectionPointers = v; }

void JVorm::removeFromVector() {
  if (v && connectionPointers) {
...
uint16 JVorm::getNumParticles() { return particles.size(); }
